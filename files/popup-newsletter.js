 ...
 * Hiện 10s sau khi đóng popup đăng nhập (#vp-popup-overlay), 1 lần/session
 */
(function () {
...
#vp-nl-box {
  ...
  position:relative;
  animation:vpNlSlideUp .4s cubic-bezier(.22,.68,0,1.2);
  text-align:center;
}
...
#vp-nl-close {
  ...
  color:#666;transition:background .2s;line-height:1;
}
...
.vp-nl-sub {
  font-size:.8rem;color:#777;line-height:1.55;margin-bottom:1.2rem;
}
.vp-nl-input {
  width:100%;padding:.78rem 1rem;
  border:1.5px solid #e0e0e0;border-radius:.65rem;
  font-size:.88rem;font-family:inherit;color:#1a1a1a;
  transition:border-color .2s;outline:none;box-sizing:border-box;
}
.vp-nl-input:focus{border-color:#c8102e}
.vp-nl-err{font-size:.76rem;color:#c8102e;margin:.3rem 0 0;display:none;text-align:left}
.vp-nl-btn {
  width:100%;padding:.82rem;margin-top:.6rem;
...
.vp-nl-skip {
  ...
  text-decoration:underline;font-family:inherit;display:block;width:100%;
}
.vp-nl-skip:hover{color:#999}
.vp-nl-success {
  font-size:.88rem;color:#2e7d32;font-weight:600;
  background:#f0faf0;border-radius:.6rem;padding:.8rem 1rem;
  margin-top:.4rem;
}
@media(max-width:480px){
  #vp-nl-box{padding:1.8rem 1.4rem 1.5rem}
  .vp-nl-title{font-size:1rem}
}
`;
  ...
  function createPopup() {
    ...
    overlay.innerHTML =
      '<div id="vp-nl-box">' +
        ...
        '<div class="vp-nl-icon"><i class="fa-solid fa-envelope-open-text" style="color:#c8102e"></i></div>' +
        ...
        '<div class="vp-nl-err" id="vp-nl-err">Vui lòng nhập email hợp lệ.</div>' +
    ...
    setTimeout(function () { document.getElementById('vp-nl-email').focus(); }, 400);
      ...
      .then(function () {
        ...
        box.innerHTML =
          '<div style="text-align:center;padding:.5rem 0">' +
            ...
            '<div class="vp-nl-sub">Cảm ơn bạn. Chúng tôi sẽ gửi thông tin hữu ích đến email của bạn sớm nhất.</div>' +
          '</div>';
  ...
  function init() {
    ...
    // Quan sát khi #vp-popup-overlay bị xóa khỏi DOM
    var observer = new MutationObserver(function (mutations) {
      for (var i = 0; i < mutations.length; i++) {
        var removed = mutations[i].removedNodes;
        for (var j = 0; j < removed.length; j++) {
          if (removed[j].id === 'vp-popup-overlay') {
            observer.disconnect();
            setTimeout(function () {
